...
Five always-local, zero-cloud-token background daemons keep the IIAL Grants
...
The audit/self-eval/improvement/self-criticism daemons are read-only with
respect to app code and data. They detect, measure, and propose; they never
mutate the app or apply changes. The watchdog mutates only daemon process state
by restarting local daemon processes. That boundary is deliberate: an
unsupervised process auto-applying LLM edits is a hazard, not a helper.
...
| `self-criticism-daemon.mjs` |  60 min | Analyzes the grant detection & extraction pipelines: reads code snippets, audit failures, and metrics. Issues 3-5 concrete criticisms of weaknesses (loose regex, missing validation, unhandled edge cases, wrong assumptions). Target: iterative improvement toward perfection. | `self-criticism-report.log`, `self-criticism-findings.md` |
...
node scripts/self-criticism-daemon.mjs  # default 60-minute cycle
node scripts/daemon-watchdog.mjs        # default 5-minute cycle
# each takes an optional [intervalMinutes] arg
```

Runtime state/output files (`*.log`, `.local-audit-report.json`,
`self-eval-metrics.jsonl`, `improvement-queue.md`,
`self-criticism-findings.md`, `.live-audit-state.json`, `.watchdog-state.json`,
`.gpu.lock`, `.pids/`, and `.supervisor.pid`) are gitignored and regenerated
every cycle.
...
`daemon-watchdog.mjs` (5-min cycle) supervises the four construction daemons
