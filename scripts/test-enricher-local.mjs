...
try {
  const pages = [
    { url: 'https://nrc.ca/main', markdown: 'Amount: $50,000 to $250,000 CAD for eligible organizations' },
    { url: 'https://nrc.ca/eligibility', markdown: 'Eligible: Canadian non-profit and for-profit organizations' },
...
try {
  const grants = [
    { funder_id: 'nserc', title: 'AI Research Fund', amount_cad_min: 50000, amount_cad_max: 250000 },
    { funder_id: 'nserc', title: 'AI Research Fund', amount_cad_min: 75000, amount_cad_max: 300000 }, // Dup attempt, different amount
    { funder_id: 'nserc', title: 'Biotech Research Fund', amount_cad_min: null, amount_cad_max: null },
  ];

  // canonicalKey = funder_id|normalizedTitle (NO amount band)
  const canonicalKeys = grants.map(g => `${g.funder_id}|${g.title.toLowerCase().replace(/\s+/g, '_')}`);

  const unique = new Set(canonicalKeys);
  assert(unique.size === 2, `Expected 2 unique (AI x2 with diff amounts = same key, Biotech = diff key), got ${unique.size}`);

  console.log(`  ✓ Duplicate detection: AI Research Fund detected as duplicate (amount band ignored)`);
