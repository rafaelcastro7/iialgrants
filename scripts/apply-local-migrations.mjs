...
async function main() {
  ...
  for (const file of files) {
    ...
    const { rows } = await client.query("SELECT 1 FROM _supabase_migrations WHERE version = $1", [
      version,
    ]);
    ...
    try {
      ...
      await client.query("INSERT INTO _supabase_migrations (version) VALUES ($1)", [version]);
