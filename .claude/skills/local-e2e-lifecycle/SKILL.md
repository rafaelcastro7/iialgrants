...
`no_knowledge_chunks: ingest org profile or documents first` if the tenant has
...
tenant has no profile and no chunks. `knowledge_chunks` is tenant-scoped: do
...
3. Alternatively, add a verified reusable answer at `/answers`; it indexes into
   the same tenant RAG corpus. Archiving it must remove it from retrieval.
