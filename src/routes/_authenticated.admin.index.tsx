...
function AdminOverview() {
  ...
  const { data: u } = useSuspenseQuery({ queryKey: ["admin", "users"], queryFn: () => fetchUsers() });
  const { data: m } = useSuspenseQuery({ queryKey: ["admin", "modules"], queryFn: () => fetchMods() });

  const totalUsers = u.users.length;
  const adminCount = u.users.filter((x) => x.is_admin).length;
  const banned = u.users.filter((x) => x.banned_until && new Date(x.banned_until) > new Date()).length;
      ...
      <Card>
        ...
        <CardContent>
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
            {m.modules.map((mod) => (
              <li key={mod.module} className="flex items-center justify-between border rounded px-3 py-2">
                <span className="font-mono text-xs">{mod.module}</span>
                <span className={mod.enabled ? "text-green-600 text-xs font-semibold" : "text-muted-foreground text-xs"}>
                  {mod.enabled ? "ON" : "OFF"}
                </span>
              </li>
            ))}
