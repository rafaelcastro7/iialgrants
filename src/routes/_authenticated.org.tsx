...
function OrgPage() {
  ...
  const [stage, setStage] = useState<(typeof STAGES)[number]>((p?.stage as (typeof STAGES)[number]) ?? "sme");
  ...
  return (
    <main className="min-h-screen bg-background text-foreground p-6">
      ...
      <Card className="max-w-2xl mx-auto">
        <CardHeader><CardTitle>{t("org.title")}</CardTitle></CardHeader>
        <CardContent>
          <form
            className="space-y-4"
            onSubmit={(e) => {
              ...
              mut.mutate({
                ...
                jurisdictions: jurisdictions.split(",").map((s) => s.trim().toUpperCase()).filter(Boolean),
                stage,
          ...
          >
            <div className="space-y-2"><Label>{t("org.name")}</Label><Input value={orgName} onChange={(e) => setOrgName(e.target.value)} required /></div>
            <div className="space-y-2"><Label>{t("org.sectors")}</Label><Input value={sectors} onChange={(e) => setSectors(e.target.value)} placeholder="tech, retail" /></div>
            <div className="space-y-2"><Label>{t("org.jurisdictions")}</Label><Input value={jurisdictions} onChange={(e) => setJurisdictions(e.target.value)} placeholder="CA, ON" /></div>
            <div className="space-y-2">
              <Label>{t("org.stage")}</Label>
              <select className="w-full border rounded h-10 px-3 bg-background" value={stage} onChange={(e) => setStage(e.target.value as (typeof STAGES)[number])}>
                {STAGES.map((s) => <option key={s} value={s}>{t(`org.stages.${s}`)}</option>)}
              </select>
            </div>
            <div className="space-y-2"><Label>{t("org.budget")}</Label><Input type="number" min="0" value={budget} onChange={(e) => setBudget(e.target.value)} /></div>
            <div className="space-y-2"><Label>{t("org.focus")}</Label><Textarea rows={3} value={focus} onChange={(e) => setFocus(e.target.value)} /></div>
            <Button type="submit" disabled={mut.isPending}>{mut.isPending ? t("app.loading") : t("org.save")}</Button>
