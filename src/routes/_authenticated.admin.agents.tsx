        ...
        {data.agents.map((a) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span
                className={`h-2 w-2 rounded-full ${a.enabled ? "bg-emerald-500" : "bg-muted-foreground/40"}`}
              />
            </div>
  ...
  async function save() {
    ...
    try {
      const promptOverride =
        draft.system_prompt.trim() === agent.builtin_prompt.trim() ? null : draft.system_prompt;
      await update({
        data: {
      ...
      <CardHeader className="border-b">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            <StatBlock
              label="Runs 24h"
              value={agent.stats.runs_24h}
              icon={<Activity className="h-3.5 w-3.5" />}
            />
            <StatBlock
              label="Success"
              value={`${Math.round(agent.stats.success_rate * 100)}%`}
              icon={<CheckCircle2 className="h-3.5 w-3.5" />}
            />
            <StatBlock
              label="Avg ms"
              value={agent.stats.avg_latency_ms}
              icon={<Gauge className="h-3.5 w-3.5" />}
            />
            <StatBlock
              label="Tokens 24h"
              value={agent.stats.total_tokens_24h.toLocaleString()}
      ...
      <CardContent className="pt-4">
          ...
          <TabsContent value="model" className="space-y-5 pt-4">
            <div className="grid md:grid-cols-2 gap-4">
                    ...
                    {models.map((m) => (
                      <SelectItem key={m.id} value={m.id}>
                        <div className="flex items-center gap-2">
                          <span>{m.label}</span>
                          <Badge
                            variant={m.tier === "free" ? "secondary" : "default"}
                            className="text-[10px]"
                          >
                            {m.tier}
                          </Badge>
              ...
              <Field label="Fallback model (on rate-limit / failure)">
                <Select
                  value={draft.fallback_model || "none"}
                  onValueChange={(v) =>
                    setDraft({ ...draft, fallback_model: v === "none" ? "" : v })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <SliderField
                label="Temperature"
                hint="Higher = more creative, less deterministic."
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <Field label="Max output tokens">
                <Input
                  ...
                  onChange={(e) =>
                    setDraft({ ...draft, max_output_tokens: Number(e.target.value) || 0 })
                  }
                />
              </Field>
              <Field label="JSON mode">
                <div className="flex items-center h-9 gap-2">
                  <Switch
                    ...
                    onCheckedChange={(v) => setDraft({ ...draft, json_mode: v })}
                  />
              ...
              <Field label="Prompt version">
                <Input
                  ...
                  onChange={(e) => setDraft({ ...draft, prompt_version: e.target.value })}
                />
              </Field>
            </div>
          </TabsContent>
        ...
        <div className="flex items-center justify-between pt-6 mt-4 border-t">
          <div className="text-xs text-muted-foreground">
            ...
            {msg && (
              <span
                className={`ml-3 inline-flex items-center gap-1 ${msg.type === "ok" ? "text-emerald-600" : "text-destructive"}`}
              >
                {msg.type === "ok" ? (
...
function StatBlock({
  ...
  return (
    <div className="text-right">
      <div className="text-[10px] uppercase text-muted-foreground flex items-center gap-1 justify-end">
        {icon} {label}
      </div>
...
function Playground({ agent }: { agent: string }) {
  ...
  return (
    <div className="space-y-3">
      <Field
        label="User message"
        hint="Sent with the current saved config (system prompt + model + params)."
      >
        <Textarea
          rows={5}
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Paste a sample grant snippet, org profile, or any test input…"
        />
      </Field>
      ...
      {result && (
        <div
          className={`rounded-lg border p-3 ${result.ok ? "" : "border-destructive/40 bg-destructive/5"}`}
        >
        ...
        {runs.map((r) => (
          <TableRow key={r.run_id}>
            ...
            <TableCell className="font-mono text-xs">
              {r.grant_id ? String(r.grant_id).slice(0, 8) : "-"}
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Every parameter and prompt edit is recorded with before / after values, the author, and a
        timestamp. Last 100 events.
      </p>
        ...
        {events.map((e) => (
          <li key={e.id} className="relative">
            ...
            <div className="flex items-center gap-2 flex-wrap text-xs">
              <Badge variant={e.is_prompt ? "default" : "secondary"} className="font-mono">
                {e.field}
              </Badge>
              <span className="text-muted-foreground">
                by {e.user_email ?? (e.user_id ? e.user_id.slice(0, 8) : "system")}
              </span>
              <span className="text-muted-foreground">
...
function PromptDiff({ oldText, newText }: { oldText: string; newText: string }) {
  ...
  for (let i = 0; i < max; i++) {
    ...
    const bv = b[i];
    if (av === bv) {
      if (av !== undefined) lines.push({ kind: "same", text: av });
      continue;
    }
    ...
    <div className="mt-1 rounded border bg-muted/30 p-2 text-[11px] font-mono max-h-72 overflow-auto">
      {lines.map((l, i) => (
        <div
          ...
          className={
            l.kind === "add"
              ? "bg-emerald-500/15 text-emerald-700 px-1"
              : l.kind === "del"
                ? "bg-destructive/15 text-destructive line-through px-1"
