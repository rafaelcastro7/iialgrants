...
import {
  ...
  makeSupabaseMock,
  type InMemoryDb,
} from "./__fixtures__/in-memory-supabase";
...
vi.mock("@/agents/llm-free.server", () => ({
  ...
  callFreeLlm: vi.fn(async () => {
    throw new Error("no_free_providers_in_test");
  }),
}));
...
const evaluatorLlm = vi.fn(async () => ({
  text: JSON.stringify({
    ...
    rationale_en:
      "Strong alignment with IIAL clean-tech capability; eligible non-profit in Ontario.",
    rationale_fr: "",
  }),
  inputTokens: 100,
  outputTokens: 50,
  ...
  db.tables.funders = [
    {
      ...
      jurisdiction: "CA",
      website: "https://innovation.ca",
      source_url: "https://innovation.ca/programs",
    },
  ];
  db.tables.grants = [
    {
...
describe("enrich → evaluate → shortlist → NotebookLM", () => {
  it(
    "enriches a grant, persists evidence, transitions discovered → enriched",
    { timeout: 30000 },
    async () => {
      ...
      expect(
        g.amount_cad_max ?? g.amount_cad_min ?? g.deadline ?? (g.sectors as string[])?.length,
      ).toBeTruthy();
  ...
  it("evaluates an enriched grant, persists verdict, transitions enriched → scored", async () => {
    ...
    const r = await evaluateGrantImpl({
      grantId: GRANT_ID,
      userId: USER_ID,
      userSupabase: supabaseMock as never,
    });
    expect(r.ok).toBe(true);
  ...
  it("re-running evaluate upserts evaluation AND dedupes evidence rows (no duplicates)", async () => {
    ...
    const enricherEvidenceCount = db.tables.evidence_spans.filter(
      (s) => s.agent !== "evaluator",
    ).length;
    ...
    expect(db.tables.evidence_spans.filter((s) => s.agent !== "evaluator").length).toBe(
      enricherEvidenceCount,
    );
  ...
  it("re-running shortlist via briefing is idempotent — no extra evidence, no duplicate events", async () => {
    ...
    // Transition events come from the grants_log_transition DB trigger, which
    // the in-memory mock doesn't emulate — app code must write ZERO of them.
    const eventsAfterFirst = db.tables.grant_events.filter(
      (e) => e.to_status === "shortlisted",
    ).length;
    expect(eventsAfterFirst).toBe(0);
    ...
    if (!second.ok)
      throw new Error(
        `second briefing failed: ${second.reason} — ${(second as { message?: string }).message ?? ""}`,
      );
    ...
    expect(db.tables.grant_events.filter((e) => e.to_status === "shortlisted").length).toBe(
      eventsAfterFirst,
  ...
  it("multi-grant briefing auto-shortlists and writes grant_events", async () => {
    ...
    // The transition event is written by the grants_log_transition DB trigger
    // (with actor via auth.uid()), not by app code — the in-memory mock has no
    // triggers, so the app-side contract here is: status updated, and NO
    // app-side grant_events insert (a second row would duplicate the trigger's).
