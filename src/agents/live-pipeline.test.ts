...
describe.skipIf(!RUN)("live pipeline", () => {
  it("enriches + evaluates a real grant end-to-end", async () => {
    ...
    const v = await evaluateGrantImpl({
      grantId: GRANT_ID!, userId: USER_ID!, userSupabase: supabaseAdmin as never,
    });
    console.log(JSON.stringify(v, null, 2));

    console.log("\n[LIVE] === FINAL GRANT ===");
    const { data: g } = await supabaseAdmin.from("grants")
      .select("status,title,summary,language,sectors,eligibility,amount_cad_min,amount_cad_max,deadline,fit_score")
