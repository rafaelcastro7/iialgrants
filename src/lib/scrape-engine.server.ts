...
async function loadRobots(origin: string): Promise<(url: string) => boolean> {
  ...
  } catch { /* missing robots = allow all */ }
...
export type ScrapeEngineResult =
  | { ok: true;  via: "scrape_engine"; url: string; title?: string; markdown: string; httpStatus: number; etag?: string; lastModified?: string; bytes: number }
  | { ok: false; via: "scrape_engine"; url: string; error: string; httpStatus?: number; gone?: boolean; blocked?: boolean; notModified?: boolean; etag?: string; lastModified?: string };
...
export async function scrapeEngineFetch(
  ...
  let parsed: URL;
  try { parsed = new URL(url); } catch { return { ok: false, via: "scrape_engine", url, error: "invalid_url" }; }
  ...
  try {
    ...
    if (!allow(url)) return { ok: false, via: "scrape_engine", url, error: "robots_disallow", blocked: true };
  } catch { /* tolerate parser errors */ }
  ...
  let res: Response;
  try {
    res = await fetch(url, { headers, signal: AbortSignal.timeout(opts.timeoutMs ?? 12_000), redirect: "follow" });
  } catch (e) {
    return { ok: false, via: "scrape_engine", url, error: e instanceof Error ? e.message : String(e) };
  }

  if (res.status === 304) {
    return {
      ok: false, via: "scrape_engine", url, error: "not_modified", httpStatus: 304, notModified: true,
      etag: res.headers.get("etag") ?? opts.etag,
      lastModified: res.headers.get("last-modified") ?? opts.lastModified,
    };
  }
  if (res.status === 404 || res.status === 410) {
    return { ok: false, via: "scrape_engine", url, error: `http_${res.status}`, httpStatus: res.status, gone: true };
  }
  if (!res.ok) {
    return { ok: false, via: "scrape_engine", url, error: `http_${res.status}`, httpStatus: res.status };
  }

  const ct = res.headers.get("content-type") ?? "";
  if (ct && !/html|xml|text/i.test(ct)) {
    return { ok: false, via: "scrape_engine", url, error: `unsupported_content_type:${ct}`, httpStatus: res.status };
  }

  let html: string;
  try { html = (await res.text()).slice(0, 1_500_000); }
  catch (e) { return { ok: false, via: "scrape_engine", url, error: `read_body_failed:${e instanceof Error ? e.message : String(e)}` }; }

  if (html.length < 200) {
    return { ok: false, via: "scrape_engine", url, error: "body_too_short", httpStatus: res.status };
  }
  ...
  if (markdown.length < 200) {
    return { ok: false, via: "scrape_engine", url, error: "extracted_too_short", httpStatus: res.status };
  }

  return {
    ok: true, via: "scrape_engine", url, title, markdown,
