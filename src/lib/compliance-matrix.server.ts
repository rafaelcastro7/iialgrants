 ...
 * Tracks mandatory sections, page limits, and policy alignment.
 */
...
interface ComplianceCheck {
  id: string;
  category: "mandatory_sections" | "format" | "eligibility" | "policy" | "financial";
  requirement: string;
...
/**
 * Canadian federal grant requirements database
 */
const REQUIREMENT_TEMPLATES: Record<string, Partial<ComplianceCheck>[]> = {
  nrc_irap: [
...
/**
 * Generate compliance matrix for a proposal
 */
export const generateComplianceMatrix = createServerFn({
  ...
  const checks: ComplianceCheck[] = template.map((req, i) => {
    let status: ComplianceCheck["status"] = "not_met";
    let details = "";

    // Check if requirement is addressed in any section
    ...
    return {
      id: `check-${i}`,
      category: req.category as ComplianceCheck["category"],
...
export { generateComplianceMatrix as generateMatrix, getRequirementTemplates };
