...
export const approveFunderCandidate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error: fErr } = await supabaseAdmin.from("funders").insert({
      ...
      name_fr: c.name_fr as string | null,
      country: "CA",
      jurisdiction: c.province as string | null,
...
export const listSourceIngestRuns = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select(
        "id,dataset,rows_in,candidates_out,auto_approved,duplicates,errors,latency_ms,status,error_message,run_at",
      )
