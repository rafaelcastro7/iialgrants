...
export const selfCheckGrant = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<SelfCheckReport> => {
    ...
    const { data: g, error } = await supabase
      .from("grants")
      .select(
        "id, title, summary, url, status, amount_cad_min, amount_cad_max, deadline, eligibility, sectors, funder_id, enriched_at, enrich_attempts, enrich_last_error, enrich_last_attempt_at",
      )
      .eq("id", data.grantId)
      .maybeSingle();
    ...
    const fields = {
      ...
      eligibility:
        !!g.eligibility && Object.keys(g.eligibility as Record<string, unknown>).length > 0,
      summary: !!g.summary && (g.summary as string).length > 40,
    };

    const issues: SelfCheckIssue[] = [];
    if (!g.url)
      issues.push({ id: "no_url", level: "error", label: "Source URL missing — cannot fetch" });
    if (!g.funder_id)
      issues.push({ id: "no_funder", level: "warn", label: "Grant not linked to a funder record" });
    if (!g.enriched_at)
      issues.push({
        id: "not_enriched",
        level: "warn",
        label: "Details not yet fetched",
        hint: "Click 'Fetch details' to scrape now",
      });
    if (g.enriched_at && !fields.summary)
      issues.push({ id: "no_summary", level: "warn", label: "Summary missing after enrichment" });
    if (g.enriched_at && !fields.amount)
      issues.push({
        id: "no_amount",
        level: "warn",
        label: "Amount missing — extractor found nothing",
      });
    if (g.enriched_at && !fields.deadline)
      issues.push({
        id: "no_deadline",
        level: "warn",
        label: "Deadline missing — page may be open-rolling",
      });
    if (g.enriched_at && (evCount ?? 0) === 0)
      issues.push({ id: "no_evidence", level: "warn", label: "No evidence citations recorded" });
    if (g.enrich_last_error)
      issues.push({
        id: "last_err",
        level: "error",
        label: `Last fetch error: ${(g.enrich_last_error as string).slice(0, 120)}`,
      });
...
export const getFetchTrail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<FetchTrail> => {
    ...
    if (row.status === "failed" && g?.enrich_last_attempt_at) {
      ...
      nextRetryAfter = new Date(
        new Date(g.enrich_last_attempt_at as string).getTime() + coolMin * 60_000,
      ).toISOString();
    }
    return {
      grantId: data.grantId,
