...
export const getFunderGivingHistory = createServerFn({ method: "GET" })
  ...
  try {
    ...
    const { data: grants, error } = await supabase
      .from("grants")
      .select(
        `id, title, amount_min, amount_max, deadline, status, sectors, jurisdictions, description`,
      )
...
export const analyzeGivingTrends = createServerFn({ method: "GET" })
  ...
  try {
    ...
    let query = supabase
      .from("grants")
      .select(
        `id, amount_min, amount_max, deadline, status, sectors, funder:funders!grants_funder_id_fkey(name, province, type)`,
      )
