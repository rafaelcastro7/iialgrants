...
export const resetAllGrants = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const e1 = await supabaseAdmin.from("evidence_spans").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    if (e1.error) throw new Error(`evidence_spans: ${e1.error.message}`);

    const e2 = await supabaseAdmin.from("grant_evaluations").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    if (e2.error) throw new Error(`grant_evaluations: ${e2.error.message}`);

    const e3 = await supabaseAdmin.from("grant_events").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    if (e3.error) throw new Error(`grant_events: ${e3.error.message}`);

    const e4 = await supabaseAdmin.from("outcomes").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    if (e4.error) throw new Error(`outcomes: ${e4.error.message}`);

    const e5 = await supabaseAdmin.from("proposals").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    if (e5.error) throw new Error(`proposals: ${e5.error.message}`);

    const e6 = await supabaseAdmin.from("submissions").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    ...
    // 3. Delete all grants
    const eg = await supabaseAdmin.from("grants").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    if (eg.error) throw new Error(`grants: ${eg.error.message}`);

    // 4. Reset crawl_ledger so re-discovery works fresh
    await supabaseAdmin.from("crawl_ledger").update({
      fetch_count: 0,
      error_count: 0,
      last_fetched_at: null,
      last_error: null,
      next_fetch_at: new Date().toISOString(),
    } as never).gte("fetch_count", 0);
