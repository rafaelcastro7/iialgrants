 ...
 * Uses TBS Proactive Disclosure data.
 */
...
export const searchCompetitiveGrants = createServerFn({ method: "GET" })
  .inputValidator(
    z.object({
      // Empty query is valid: it matches all rows (ilike '%%'), which the
      // Program Analysis view relies on to load the full landscape.
...
export const findCompetitors = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      const byRecipient: Record<
        string,
        { count: number; totalValue: number; programs: Set<string> }
      > = {};
      for (const r of competitors || []) {
        const name = r.recipient_name;
        if (!byRecipient[name])
          byRecipient[name] = { count: 0, totalValue: 0, programs: new Set() };
        byRecipient[name].count++;
        byRecipient[name].totalValue += r.agreement_value || 0;
        if (r.program_name) byRecipient[name].programs.add(r.program_name);
      }
