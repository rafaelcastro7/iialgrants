...
const FunderEnrichmentSchema = z.object({
  funderId: z.string().uuid(),
  website: z.string().url().optional(),
});

type FunderEnrichmentInput = z.infer<typeof FunderEnrichmentSchema>;
 ...
 */
async function scrapeFunderWebsite(url: string): Promise<EnrichmentResult["data"]> {
  try {
    ...
    const response = await fetch(url, {
      ...
      headers: {
        "User-Agent": "IIAL-GrantIntelligence/1.0 (research)",
      },
    });
    ...
    // Extract meta description as mission statement
...
export const enrichFunder = createServerFn({
  method: "POST",
  validator: FunderEnrichmentSchema,
}).handler(async ({ data }) => {
