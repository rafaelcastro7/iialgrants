...
import { CheckCircle2, AlertCircle, Loader2, Clock, X, Download, FileJson, FileSpreadsheet } from "lucide-react";
...
type ErrorCategory = "timeout" | "rate_limit" | "http" | "parse" | "auth" | "network" | "llm" | "other";
...
function categorizeError(msg: string): { category: ErrorCategory; hint: string; hintFr: string } {
  ...
  if (m.includes("timeout") || m.includes("timed out") || m.includes("etimedout")) {
    return { category: "timeout", hint: "Source took >90s. Try fewer funders or increase timeout.", hintFr: "La source a pris >90s. Essayez moins de fournisseurs ou augmentez le délai." };
  }
  if (m.includes("429") || m.includes("rate") || m.includes("quota")) {
    return { category: "rate_limit", hint: "Rate-limited. Wait a minute before retrying.", hintFr: "Limite atteinte. Attendez une minute avant de réessayer." };
  }
  if (m.includes("401") || m.includes("403") || m.includes("unauthor") || m.includes("forbidden") || m.includes("api key")) {
    return { category: "auth", hint: "Auth failed. Check Firecrawl / Lovable AI credentials.", hintFr: "Échec d'authentification. Vérifiez les clés Firecrawl / Lovable AI." };
  }
  if (m.includes("404") || m.includes("not found")) {
    return { category: "http", hint: "Source URL returned 404. Update funder.source_url.", hintFr: "URL source retourne 404. Mettez à jour funder.source_url." };
  }
  if (m.includes("5") && (m.includes("500") || m.includes("502") || m.includes("503") || m.includes("504"))) {
    return { category: "http", hint: "Upstream server error. Retry later.", hintFr: "Erreur serveur en amont. Réessayez plus tard." };
  }
  if (m.includes("json") || m.includes("parse") || m.includes("schema") || m.includes("zod")) {
    return { category: "parse", hint: "LLM output failed schema validation. Inspect run metadata.", hintFr: "Sortie LLM invalide. Inspectez les métadonnées de la corrida." };
  }
  if (m.includes("fetch") || m.includes("network") || m.includes("econn") || m.includes("dns")) {
    return { category: "network", hint: "Network failure reaching the source.", hintFr: "Échec réseau vers la source." };
  }
  if (m.includes("gemini") || m.includes("llm") || m.includes("model")) {
    return { category: "llm", hint: "LLM call failed. Try again or fall back to Gemini Pro.", hintFr: "Appel LLM échoué. Réessayez ou utilisez Gemini Pro." };
  }
  return { category: "other", hint: "See Event Log for full trace.", hintFr: "Voir le journal d'événements pour la trace complète." };
}
...
export function DiscoveryProgress({
  ...
  const errorGroups = useMemo(() => {
    const groups = new Map<ErrorCategory, { hint: string; funders: { name: string; msg: string }[] }>();
    for (const f of perFunder) {
  ...
  function exportJson() {
    ...
    downloadBlob(JSON.stringify(payload, null, 2), "application/json", `discovery-${jobId.slice(0, 8)}.json`);
  }
  function exportCsv() {
    const header = ["funder_id", "funder_name", "status", "attempts", "inserted", "seen_again", "engine", "latency_ms", "last_error"];
    const rows = perFunder.map((f) => [
      f.funder_id, f.funder_name, f.status, f.attempts, f.inserted, f.seenAgain,
      f.engine ?? "", f.latency_ms ?? "", (f.lastError ?? "").replace(/"/g, '""'),
    ]);
    ...
    <section className="mb-4 border rounded-lg bg-card overflow-hidden">
      <header className="flex items-center justify-between px-4 py-2 border-b bg-muted/30">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" onClick={exportJson} disabled={!data} title="Export JSON">
            <FileJson className="h-3.5 w-3.5" />
          </Button>
          <Button variant="ghost" size="sm" onClick={exportCsv} disabled={perFunder.length === 0} title="Export CSV">
            <FileSpreadsheet className="h-3.5 w-3.5" />
          </Button>
          <button type="button" onClick={onClose} className="text-muted-foreground hover:text-foreground ml-1" aria-label="Close">
            <X className="h-4 w-4" />
          </button>
        </div>
      </header>
        ...
        <div className="px-4 py-2 border-y bg-destructive/5 space-y-1.5">
          ...
          {errorGroups.map(([cat, g]) => (
            <div key={cat} className="text-xs">
              ...
              <span className="text-muted-foreground">{g.funders.length} · {g.hint}</span>
              <span className="text-muted-foreground ml-1">— {g.funders.map((f) => f.name).join(", ")}</span>
            </div>
          ))}
        </div>
      )}
        ...
        {visible.length === 0 && (
          <li className="px-4 py-3 text-xs text-muted-foreground">
            {perFunder.length === 0
              ...
              : (fr ? "Aucun fournisseur dans cette vue." : "No funders match this view.")}
          </li>
        )}
        {visible.map((f) => (
      ...
      <p className="px-4 py-1.5 text-[10px] text-muted-foreground border-t bg-muted/10">
        {fr ? "Détails complets par réintento dans le Event Log ci-dessous." : "Full per-retry details in the Event Log below."}
      </p>
    </section>
  );
}
